# CVSight 0.1.0 release guide

CVSight 0.1.0 is a self-hosted, single-project image annotation platform for dense retail
shelves. PostgreSQL and local managed media are authoritative. The tested deployment is
Windows 11 with PowerShell 7, Docker Desktop, Python 3.13, uv, and Node.js 24.
...
source control. Bind local services to loopback unless an operator-managed HTTPS reverse
proxy supplies access controls and login rate limiting.
...
1. An owner creates a dataset and imports authorized JPEG or PNG shelf images.
2. An annotator optionally requests RF-DETR pre-label jobs, then verifies every proposed
   box. A human can also draw, resize, reject, flag, or duplicate boxes.
...
6. An owner exports deterministic detection or recognition archives and reads analytics
   tied to the immutable snapshot and formula version.
...
`LICENSE_POLICY.md` records the dependency, model, dataset, and shipped-artifact posture.
The CVSight source currently has no license grant and must not be described as open source.

## Known limitations

- The tested operational path is Windows 11 and PowerShell 7. Linux scripts are not
  included in this release.
- The built-in server is local infrastructure, not an internet-facing authentication
  edge. HTTPS termination and login rate limiting belong to the reverse proxy.
...
- GPU checkpoints are mounted local files. Registry promotion does not distribute a
  model artifact or restart a worker.
- RF-DETR, CLIP, and SAM results remain proposals or suggestions. No model may assign a
  SKU automatically.
...
- Human-effort baselines and active-learning effectiveness remain deferred until the
  required controlled labeling cycles and eligible model candidate exist.
